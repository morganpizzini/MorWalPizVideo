...
const ImportVideo: React.FC = () => {
  ...
  const { categories: availableCategories } = useLoaderData<LoaderData>();
  ...
  useEffect(() => {
    ...
    if (result.success) {
      toast.show('Success', 'Video imported successfully', { variant: 'success' });
      // Reset form
      setVideoId('');
      setCategories([]);
    }
  ...
  const isDisabled = () => !videoId || categories.length === 0 || busy;

  return (
    <>
      <PageHeader title="Import Video" />
