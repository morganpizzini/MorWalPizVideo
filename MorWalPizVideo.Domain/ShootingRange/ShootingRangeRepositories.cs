...
public class MongoShootingRangeRepository<T>(IMongoDatabase database, string collectionName) : IShootingRangeRepository<T> where T : BaseEntity
{
    ...
    public Task<T?> GetAsync(string id, CancellationToken cancellationToken = default) => collection.Find(Builders<T>.Filter.Eq("_id", id)).FirstOrDefaultAsync(cancellationToken);
...
public sealed class MongoShootingRangeUserRepository(IMongoDatabase database) : MongoShootingRangeRepository<ShootingRangeUser>(database, "shootingRangeUsers"), IShootingRangeUserRepository
{
    ...
    public Task<ShootingRangeUser?> FindByUsernameAsync(string username, CancellationToken cancellationToken = default) => collection.Find(x => x.Username == username).FirstOrDefaultAsync(cancellationToken);
}
