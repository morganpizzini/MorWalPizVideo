...
import { InsightCommentSourceType } from '@morwalpizvideo/models';
...
export default function InsightCommentsPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setSubmitting(true);
    try { setResult(await insightsTopicsApi.analyzeComments(topicId, { sourceType, channelId: sourceType !== InsightCommentSourceType.DirectVideoId ? channelId : undefined, videoId: sourceType === InsightCommentSourceType.StoredChannel ? undefined : videoId.trim(), commentsNumber })); }
  ...
  return <div className="rbac-form-width">
    ...
    <Form onSubmit={submit} className="d-grid gap-3">
      ...
      {sourceType === InsightCommentSourceType.StoredChannel ? null : <Form.Group><Form.Label htmlFor="insight-video">YouTube video ID</Form.Label><Form.Select id="insight-video" value={videoId} onChange={event => setVideoId(event.target.value)} required={sourceType === InsightCommentSourceType.StoredVideo}><option value="">Select a stored video</option>{sourceType === InsightCommentSourceType.StoredVideo ? (channels.find(channel => channel.channelId === channelId)?.videos ?? []).map(video => <option key={video.videoId} value={video.videoId}>{video.title || video.videoId}</option>) : null}</Form.Select>{sourceType === InsightCommentSourceType.DirectVideoId ? <Form.Control className="mt-2" aria-label="Direct YouTube video ID" value={videoId} onChange={event => setVideoId(event.target.value)} required /> : null}</Form.Group>}
