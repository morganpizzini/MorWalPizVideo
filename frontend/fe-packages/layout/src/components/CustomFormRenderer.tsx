...
export function CustomFormRenderer({
  ...
  const updateAnswer = (question: AnyQuestion, answer: AnyAnswer) =>
    setAnswers((previous) => ({ ...previous, [question.questionId]: answer }));

  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const orderedAnswers = form.questions
      .slice()
      .sort((left: AnyQuestion, right: AnyQuestion) => left.order - right.order)
      .map(
        (question: AnyQuestion) =>
          answers[question.questionId] ?? emptyAnswer(question),
      );
    const unanswered = form.questions.find((question: AnyQuestion) => {
      const answer = orderedAnswers.find(
        (item: AnyAnswer) => item.questionId === question.questionId,
      )!;
    ...
    if (unanswered) {
      setError(
        `Please answer the required question: "${unanswered.questionText}"`,
      );
      return;
    }
  ...
  return (
    <form onSubmit={submit}>
      ...
      {form.questions
        .slice()
        .sort(
          (left: AnyQuestion, right: AnyQuestion) => left.order - right.order,
        )
        .map((question: AnyQuestion, index: number) => {
              ...
              {(question.questionType === QuestionType.SingleChoice ||
                ...
                question.options.map(
                  (option: { optionId: string; optionText: string }) => {
                    const checked =
                      question.questionType === QuestionType.SingleChoice
                        ? answer.answerType === AnswerType.SingleChoice &&
                          answer.selectedOptionId === option.optionId
                        : answer.answerType === AnswerType.MultipleChoice &&
                          answer.selectedOptionIds.includes(option.optionId);
                    return (
                      <label
                        key={option.optionId}
                        className="d-block form-check mb-2"
                      >
                        <input
              ...
              {question.questionType === QuestionType.Boolean && (
                <label className="form-check">
                  <input
                    ...
                    type="checkbox"
                    checked={
                      answer.answerType === AnswerType.Boolean && answer.value
                    }
                    onChange={(event) =>
                      updateAnswer(question, {
                        _t: "BooleanAnswer",
