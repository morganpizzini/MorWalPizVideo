...
| TD-010 | High | Short links are split across three storage locations | No global uniqueness, scans, lost counts | Use standalone collection and atomic counters in the unpublished target | High | Target decision: standalone shortlinks only. No legacy embedded links, migration endpoint, backfill, or dual read is supported. |
