...
|---|---|---|---|---|
| 1 | TD-009 | Partially addressed | CI skipped actual backend tests | CI now runs `MorWalPizVideo.BackOffice.Tests`; AppHost and YouTubeUtilities test coverage remain omitted, while the shop omission is intentional during the hold. |
...
| TD-009 | High | CI coverage omits active verification paths | Regressions reach deployment | Add AppHost build verification, YouTubeUtilities tests, and non-empty tests for deployed active clients | Medium | Partially addressed — BackOffice tests and principal builds run; shop remains intentionally excluded while on hold |
