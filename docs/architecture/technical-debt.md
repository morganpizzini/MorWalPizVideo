...
| TD-010 | High | Short links use standalone records for channel/generic links and embedded match records for YouTube videos | Video-link scans and split ownership complicate uniqueness, migration, and reporting | Keep standalone codes globally indexed, preserve video-reference validation, then migrate video links with an idempotent backfill | High | In progress — the current compatibility model is intentional and covered by focused mock-scenario tests; standalone-only ownership remains a future migration. |
...
| TD-013 | Medium | Broad `DataService` has excessive dependencies and responsibility | Coupling and difficult tests | Extract focused feature services incrementally | High | Partially addressed — focused services cover content, catalog, shop, forms, insights, and links; remaining `DataService` consumers continue as separate migration slices and compatibility registrations remain intentional. |
...
| TD-016 | Medium | ShortLinks uses read-modify-replace counting and full scans for embedded video links | Lost updates and scaling failure on the embedded compatibility path | Indexed lookup and atomic increment, followed by video-link migration | Medium | Partially closed — standalone links resolve via `GetShortLinkByCode` and increment atomically via `IncrementClicksAsync` (`FindOneAndUpdate` + `$inc`); embedded video links remain on the compatibility path until backfilled. |
...
| TD-020 | Medium | Blob abstraction loses metadata and swallows failures | Incorrect media responses and weak diagnostics | Return typed blob metadata/result and classify failures | Medium | Partially addressed — `BlobDownloadResult` and the deterministic mock now cover metadata, not-found, checksum, transient, permanent, malformed, and cancellation outcomes; production/provider hardening remains open. |
