...
public sealed class MockShootingRangeBookingRepository : MockShootingRangeRepository<ShootingRangeBooking>, IShootingRangeBookingRepository
{
    ...
    public async Task<bool> HasOverlapAsync(string bayId, DateTime startUtc, DateTime endUtc, CancellationToken cancellationToken = default)
    {
        await gate.WaitAsync(cancellationToken);
        try { return Items.Values.Any(x => x.BayId == bayId && x.StartUtc < endUtc && x.EndUtc > startUtc && x.Status is not ShootingRangeBookingStatus.Rejected and not ShootingRangeBookingStatus.Cancelled); }
        finally { gate.Release(); }
    }

    public async Task<ShootingRangeBooking> InsertIfAvailableAsync(ShootingRangeBooking booking, CancellationToken cancellationToken = default)
    {
        await gate.WaitAsync(cancellationToken);
        try
        {
            if (Items.Values.Any(x => x.BayId == booking.BayId && x.StartUtc < booking.EndUtc && x.EndUtc > booking.StartUtc && x.Status is not ShootingRangeBookingStatus.Rejected and not ShootingRangeBookingStatus.Cancelled)) throw new InvalidOperationException("The selected slot is already pending or booked.");
            Items[booking.Id] = booking;
            return booking;
        }
        finally { gate.Release(); }
    }
