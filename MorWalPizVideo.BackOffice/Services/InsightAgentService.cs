...
#pragma warning restore SKEXP0010
            ...
            string description = "")
        {
            ...
            var prompt = BuildCommentsPrompt(topic, videoTitle, seedArgumentsString, commentsBlock, description, sourceKind);
...
#pragma warning restore SKEXP0010
            ...
            var result = await _kernel.InvokePromptAsync(trimmedPrompt, new KernelArguments(executionSettings));
            var analysis = JsonSerializer.Deserialize<VideoCommentAnalysisResponse>(result.ToString(), new JsonSerializerOptions
            {
            ...
            foreach (var idea in analysis.Ideas)
            {
                ...
                newsItems.Add(new InsightNewsItem(
                    ...
                    starRating: 0,
                    aiRelevanceScore: NormalizeRelevanceScore(idea.RelevanceScore),
        ...
        internal static string BuildCommentsPrompt(
            ...
            InsightSourceKind sourceKind = InsightSourceKind.ShortContent)
        {
            var promptDescription = NormalizeVideoDescription(description);

            return @$"
Analizza i seguenti commenti raccolti su un video YouTube. Usa tutto il contesto del video per valutare l'affinita delle idee.

Titolo video: {videoTitle}
Descrizione video (contesto transitorio, non persistente):
{promptDescription}
Topic: {topic.Title}
Seed arguments: {seedArgumentsString}
...
{{
    ""ideas"": [
      ...
      ""idea"": ""descrizione dell'idea per il contenuto"",
            ...
            ""relevanceScore"": numero tra 0 e 1 che misura l'affinita dell'idea con il topic
    }}
    ]
}}
Il relevanceScore deve essere sempre compreso tra 0 e 1. Se nessun commento contiene idee, restituisci un array 'ideas' vuoto.";
        }

        internal static string NormalizeVideoDescription(string? description)
        {
        ...
        internal static double NormalizeRelevanceScore(JsonElement? value)
        {
            ...
            double score;
            if (element.ValueKind == JsonValueKind.Number && element.TryGetDouble(out var numericScore))
            {
                score = numericScore;
            }
            ...
            string description = "")
        {
            ...
            for (int i = 0; i < ideaCount; i++)
            {
                ...
                newsItems.Add(new InsightNewsItem(
                    ...
                    status: InsightNewsStatus.AutoDetected,
                    starRating: 0,
                    aiRelevanceScore: Math.Round(0.65 + (random.NextDouble() * 0.2), 2),
                    discoveredAt: DateTime.UtcNow,
                    platformSource: "YouTube",
                    postId: videoId,
                    videoId: videoId,
